...
public class BookControllerAPI {
    ...
    public ResponseEntity<DataResponse<Book>> byId(@RequestParam Long id){
        try {
            return ResponseEntity.ok(
                    new DataResponse<>(true, "Найдена следующая книга", bookService.findById(id))
            );
        } catch (Exception e) {
            return ResponseEntity.ok(
    ...
    @PostMapping
    public ResponseEntity<DataResponse<Book>> save(@RequestBody BookDto book){
        try {
            return ResponseEntity.ok(
    ...
    @PutMapping
    public ResponseEntity<BaseResponse> update(@RequestParam Long id, @RequestBody BookDto book){
        try {
            bookService.update(bookService.findById(id), book);
            return ResponseEntity.ok(
